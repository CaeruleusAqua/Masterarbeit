...
\todo{beleg} For this reason, a ground plane based algorithm is implemented for the segmentation.
...
For this purpose, a tracking and state estimation algorithm is developed in section \todo{section reference}, which should grant that.
...
\todo{beleg} beacuse the gradients are too small and the depletion of the necessary thresholds leads to frequent false positives. 
...
\todo{iterativen ablauf des ganzen Algorythmusses klar machen}
...
This is stored asdrrum in a static two-dimensional array: Points [2000] [16]. The order of the data is keeped thereby to allow efficient access to 
...
\todo{wie ergibt sich höhe und breite! (aus der orientierung.... erklären)}
\begin{itemize}
...
\todo{kalman nicht nur für Prediction auch für filterung}
...
From the previous tracking, we can get the current position, speed, rotation and rotation speed. For an estimation of the state with the for vehicle common the bicycle model \todo{reference}, 
...
the measurement data have many holes on them, so they must be transformed into a suitable form.
...
It is important to note that object tracking requires a complete 360 degree measurement. To do this, you must wait until all necessary measurement data are available.
...
the measured values are rounded down to the original resolution of 0.2 or 2 degrees and the measured values are entered into a corresponding two-dimensional fixed-size array.
...
to formulate different traffic situations. The SCNX format offers, among other things, the classes ROAD, LANE for the modeling of roads. One ROAD can consist of several LANE's.
A lane consists of a set of points which describe the course of the lane. The lane can still be assigned as an attribute lane mark and a width. 
...
At the same time, only the classes required for the simulation are implemented. Therefore, a lane can only be described by a point model, while OpenDAVINCI offers additional models.
